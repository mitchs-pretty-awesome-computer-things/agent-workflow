...
- `<prefix>:human-in-the-loop` — requires a human step before automation can continue.
...
- `--version <tag>` installs a specific MAW version (e.g. `--version v0.2.0`); it defaults to `latest`.
